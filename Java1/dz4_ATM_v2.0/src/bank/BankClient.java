...
public class BankClient {
    ...
    public void queueOperation(BankClientOperationTypes type, double sum) {
        BankClientOperations operation;
        switch (type) {
            case WITHDRAWAL:
                operation = new BankClientOperations(BankClientOperationTypes.WITHDRAWAL, sum);
                operation.setOperatedCard(card);
                operationsHistory.add(operation);
                balance.withdrawalOperation(sum);
                break;
            case ADD:
                operation = new BankClientOperations(BankClientOperationTypes.ADD, sum);
                operation.setOperatedCard(card);
                operationsHistory.add(operation);
