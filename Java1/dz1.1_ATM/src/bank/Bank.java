...
import bank.dataBases.IBankDataBase;
...
public abstract class Bank implements IBank {
    ...
    private IBankResponse getCardBalance(BankClient client) {
        return new BankResponse(BankResponses.BALANCE, BALANCE_MESSAGE + client.getBalance());
    }
    ...
    private BankResponse processDebitPayment(ClientRequisites costumer, double sum) {
        ...
        if (client.getBalance() < sum) {
            response = new BankResponse(BankResponses.DENIED, NO_MONEY_WITHDRAWAL_MESSAGE);
        } else {
            client.queueOperation(BankClientOperationTypes.WITHDRAWAL, sum);
            response = new BankResponse(BankResponses.ACCEPTED, ACCEPTED_MESSAGE);
        }
        return response;
    }
    ...
    private BankResponse processCreditPayment(ClientRequisites costumer, double sum) {
        ...
        if (client.getBalance() < sum) {
            if (creditOperationPossible(client, sum)) {
                ...
                client.queueOperation(BankClientOperationTypes.WITHDRAWAL, sum);
                response = new BankResponse(BankResponses.ACCEPTED_CREDIT, ACCEPTED_AS_CREDIT_MESSAGE);
            }
        } else {
            client.queueOperation(BankClientOperationTypes.WITHDRAWAL, sum);
            response = new BankResponse(BankResponses.ACCEPTED, ACCEPTED_MESSAGE);
    ...
    public IBankResponse showCredit(ClientRequisites costumer) {
        BankClient client = getClient(costumer);
        return new BankResponse(BankResponses.SHOW_CREDIT, CREDIT_MESSAGE + client.getCurrentCredit());
    }

    @Override
    public IBankResponse showCardHistory(ClientRequisites costumer) {
        IBankResponse result = new BankResponse(BankResponses.HISTORY);
    ...
    private IBankResponse addMoney(ClientRequisites costumer, BankRequest request) {
        BankClient client = getClient(costumer);
        client.queueOperation(BankClientOperationTypes.ADD, request.getSum());
        return new BankResponse(BankResponses.ACCEPTED_MONEY_ADDITION, MONEY_ADDED_MESSAGE);
    }
}
